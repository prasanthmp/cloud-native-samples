...
resource "null_resource" "tag_oke_lbs_grafana" {
  ...
  provisioner "local-exec" {
    command = "${path.module}/tag_oke_lbs.sh ${var.compartment_ocid} ${oci_containerengine_cluster.oke-selfmanaged-cluster.id} ${var.kubernetes.cluster_name}-grafana ${data.external.grafana_ip.result.external_ip}"
    }
