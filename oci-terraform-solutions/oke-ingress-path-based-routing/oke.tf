resource oci_containerengine_cluster oke-cluster {
  ...
  options {
    ...
    service_lb_subnet_ids = [
      oci_core_subnet.oke-cluster-loadbalancer-subnet-regional.id,
    ]
  }
}
  ...
  node_config_details {
    size = var.kubernetes.number_of_nodes # Number of worker nodes
      placement_configs {
        ...
        subnet_id           = oci_core_subnet.oke-cluster-nodesubnet-regional.id
      }
    node_pool_pod_network_option_details {
      ...
      pod_subnet_ids = [
        oci_core_subnet.oke-cluster-nodesubnet-regional.id,
      ]
    }   
   }
