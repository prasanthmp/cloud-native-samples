# Oracle Kubernetes Engine (OKE) with Auto Scaling, Prometheus & Grafana

This repository provides Terraform configurations for deploying a fully functional Oracle Kubernetes Engine (OKE) cluster on Oracle Cloud Infrastructure (OCI), complete with Cluster Autoscaler, Prometheus, and Grafana for automated scaling and observability.

The setup is designed to serve as a reproducible, end-to-end example of modern Kubernetes infrastructure management on OCI. It demonstrates how to:

- Provision an OKE cluster using Terraform for consistent, infrastructure-as-code deployment.
- Enable Cluster Autoscaler to dynamically adjust the number of worker nodes based on workload demands, optimizing both performance and cost.
- Deploy Prometheus for metrics collection and alerting from Kubernetes nodes, pods, and cluster components.
- Visualize metrics in Grafana, providing real-time dashboards for cluster health, workload performance, and resource utilization.

## What this repo does
- Automated OKE cluster with autoscaler provisioning using Terraform
...
- `cluster-autoscaler.tf` - Cluster autoscaler addon deployment
...
- `load-test.yaml` - App deployment for testing OKE scale-up/scale-down
...
kubectl get nodes
```
Go to Grafana dashboard and check node size (It should decrease to 2 (using default load-test.yaml))
...
## How to check cluster autoscaler logs
...
Edit terraform.tfvars.local file
